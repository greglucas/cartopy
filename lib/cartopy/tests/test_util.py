...
class Test_add_cyclic_point:
    ...
    def test_data_and_coord(self):
        ...
        r_data = np.concatenate((self.data2d, self.data2d[:, :1]), axis=1)
        r_lons = np.concatenate((self.lons, np.array([360.0])))
        assert_array_equal(c_data, r_data)
...
class TestAddCyclic:
    ...
    def setup_class(cls):
        ...
        cls.c_lons = np.concatenate((cls.lons, np.array([360.0])))
        cls.c_lon2d = np.concatenate(
            (cls.lon2d, np.full((cls.lon2d.shape[0], 1), 360.0)), axis=1
        )
        cls.c_lon3d = np.concatenate(
            (cls.lon3d, np.full((cls.lon3d.shape[0], 1, cls.lon3d.shape[2]), 360.0)),
            axis=1,
        )
        cls.c_lats = cls.lats
    ...
    def test_masked_data_and_x_y_2d(self):
        ...
        r_lons = np.concatenate(
            (self.lon2d, np.full((self.lon2d.shape[0], 1), 360.0)), axis=1
        )
        assert_array_equal(c_data, r_data)
        assert_array_equal(c_lons, r_lons)
        assert_array_equal(c_lats, self.c_lat2d)
    ...
    def test_cyclic(self):
        ...
        c_data, c_lons, c_lats = add_cyclic(
            self.data4d, x=new_lons, y=new_lats, axis=1, cyclic=np.deg2rad(360.0)
        )
        r_lons = np.concatenate(
            (
                new_lons,
                np.full((new_lons.shape[0], 1, new_lons.shape[2]), np.deg2rad(360.0)),
            ),
            axis=1,
        )
    ...
    def test_cyclic_has_cyclic(self):
        ...
        r_data = np.concatenate((self.data2d, self.data2d[:, :1]), axis=1)
        r_lons = np.concatenate(
            (new_lons, np.full((new_lons.shape[0], 1), np.deg2rad(360.0))), axis=1
        )
        r_lats = np.concatenate((new_lats, new_lats[:, -1:]), axis=1)
        c_data, c_lons, c_lats = add_cyclic(
            r_data, x=r_lons, y=r_lats, cyclic=np.deg2rad(360.0)
        )
        assert_array_equal(c_data, self.c_data2d)
        assert_array_equal(c_lons, r_lons)
        assert_array_equal(c_lats, r_lats)
    ...
    def test_precision_has_cyclic(self):
        ...
        r_data = np.concatenate((self.data2d, self.data2d[:, :1]), axis=1)
        r_lons = np.concatenate((self.lons, np.array([360.0 + 1e-3])))
    ...
    def test_precision_has_cyclic_no(self):
        ...
        r_lons = np.concatenate((new_lons, np.array([360.0])))
        assert_array_equal(c_data, r_data)
        assert_array_equal(c_lons, r_lons)
...
class TestHasCyclic:
    ...
    # cyclic lon 1d, 2d, 3d
    c_lons = np.concatenate((lons, np.array([360.0])))
    c_lon2d = np.concatenate((lon2d, np.full((lon2d.shape[0], 1), 360.0)), axis=1)
    c_lon3d = np.concatenate(
        (lon3d, np.full((lon3d.shape[0], 1, lon3d.shape[2]), 360.0)), axis=1
    )
    ...
    def test_3d_axis_cyclic(self):
        ...
        new_lons = np.deg2rad(self.lon3d)
        assert has_cyclic(new_clons, axis=1, cyclic=np.deg2rad(360.0))
        assert not has_cyclic(new_lons, axis=1, cyclic=np.deg2rad(360.0))

    def test_1d_precision(self):
        """Test 1d with precision keyword"""
        new_clons = np.concatenate((self.lons, np.array([360.0 + 1e-3])))
