...
_TEST_DATA_DIR = os.path.join(config["data_dir"], "wmts", "aerial")
...
def test_intersect(tmp_path):
    ...
    for zoom, image_names in expected:
        ...
        fnames = sorted([os.path.basename(item[1].filename) for item in ancestry])
...
def test_nest(nest_from_config):
    ...
    z0.scan_dir_for_imgs(
        os.path.join(_TEST_DATA_DIR, "z_0"), glob_pattern="*.png", img_class=RoundedImg
    )

    z1 = cimg_nest.ImageCollection("aerial z1 test", crs)
    z1.scan_dir_for_imgs(
        os.path.join(_TEST_DATA_DIR, "z_1"), glob_pattern="*.png", img_class=RoundedImg
    )

    z2 = cimg_nest.ImageCollection("aerial z2 test", crs)
    z2.scan_dir_for_imgs(
        os.path.join(_TEST_DATA_DIR, "z_2"), glob_pattern="*.png", img_class=RoundedImg
    )
