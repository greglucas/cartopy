...
def test_Downloader_data():
    di = cio.Downloader(
        ...
        os.path.join("{data_dir}", "{category}", "shape.shp"),
    ...
    assert di.target_path(replacement_dict) == os.path.join(
        "/wibble", "foo", "bar", "example", "shape.shp"
    )

    assert (
        di.pre_downloaded_path(replacement_dict) == "/project/foobar/example/sample.shp"
    )
