...
cimport cython
import numpy as np
from libc.math cimport HUGE_VAL, sqrt, pi
...
def project_linear(geometry not None, src_crs not None,
    ...
    print(resample_chain(geometry.coords[:], src_crs, dest_projection))
...
cdef (double, double) spherical(double x, double y, double z):
    """
...
cdef (double, double, double) cartesian(double lon, double lat):
    """
...
cdef double stereo_length(double x0, double y0, double z0,
                  double x1, double y1, double z1):
    """
    Compute the stereographic length between cartesian points
    (x0, y0, z0) -> (x1, y1, z1)
    """
    cdef double pxy, pyz, pzx, q
...
cdef double stereo_area(double x0, double y0, double z0,
                double x1, double y1, double z1,
...
cdef double planar_midpoint(double x0, double y0, double x1, double y1):
    """
...
cdef void resample_line(double w0x, double w0y,
                  dobule u0x, double u0y, double u0z,
                  double w1x, double w1y,
                  double u1x, double u1y, double u1z,
                  double dist01, Transformer transformer,
                  int depth, list[(double, double)] array):
    """
    w == input coordinates
    u == cartesian coordinates on the sphere
    dist == stereographic distance squared
    depth == current depth
    array == array of coordinates
    """
    cdef double dist02, dist12, AA, hh, ww, p2x, p2y, u2x, u2y, u2z
    depth -= 1
    if not depth:
        return

    w2x, w2y = planar_midpoint(w0x, w0y, w1x, w1y)
    p2x, p2y = transformer.transform(w2x, w2y)
    u2x, u2y, u2z = cartesian(p2x, p2y)
    # u2 = cartesian(*w2)
    dist02 = stereo_length(u2x, u2y, u2z, u0x, u0y, u0z)
    dist12 = stereo_length(u2x, u2y, u2z, u1x, u1y, u1z)
    AA = stereo_area(u2x, u2y, u2z, u0x, u0y, u0z, u1x, u1y, u1z)
    hh = AA * (1 + 0.25*dist01)*(1 + 0.25*dist01) / (dd * dist01) # perpendicular projected distance
    ww = 2 * ((dist02 - dist12) / dist01) * ((dist02 - dist12) / dist01) # midpoint roughly in the middle
    # (dist02 + dist12 > 0.25) forces bisection for over-long segments, and handles dist01 == Infinity edge case
    # (dist02 + dist12 > dd)) stops bisection when the segment gets tiny
    if (((hh + ww > 1) and (dist02 + dist12 > dd)) or (dist02 + dist12 > 0.25)):
        # Go down the first half of the segment (p0 -> p2)
        resample_line(w0x, w0y, u0x, u0y, u0z, w2x, w2y, u2x, u2y, u2z, dist02, transformer, depth, array)
        # Append the middle point
        array.push_back((p2x, p2y))
        # Go down the second half of the segment (p2 -> p1)
        resample_line(w2x, w2y, u2x, u2y, u2z, w1x, w1y, u1x, u1y, u1z, dist12, transformer, depth, array)


def resample_chain(list point_array, proj_from not None, proj_to not None):
    cdef readonly transformer = Transformer.from_crs(proj_from, proj_to, always_xy=True)
    cdef double w0x, w0y, w1x, w1y, u0x, u0y, u0z, u1x, u1y, u1z, p0x, p0y, p1x, p1y
    cdef unsigned int i
    cdef list[(double, double)] outarray
    w0x, w0y = point_array[0]
    # p0 is the transformed point in spherical coordinates
    p0x, p0y = transformer.transform(w0x, w0y)
    u0x, u0y, u0z = cartesian(p0x, p0y)
    outarray = []
    outarray.push_back((p0x, p0y))
    # Iterate over all points in the line
    for i in range(1, len(point_array)):
        w1x, w1y = point_array[i]
        p1x, p1y = transformer.transform(w1x, w1y)
        u1x, u1y, u1z = cartesian(p1x, p1y)
        resample_line(w0x, w0y, u0x, u0y, u0z, w1x, w1y, u1x, u1y, u1z, stereo_length(u0x, u0y, u0z, u1x, u1y, u1z),
                      transformer, maxDepth, outarray)
        outarray.push_back((p1x, p1y))
        w0x, w0y = w1x, w1y
