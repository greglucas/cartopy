...
def regrid(
    ...
    xyz = source_cs.transform_points(
        source_cs, source_x_coords.flatten(), source_y_coords.flatten()
    )
    # Transform the target points into the source projection
    target_xyz = source_cs.transform_points(
    ...
    back_to_target_xyz = target_proj.transform_points(
        source_cs, target_xyz[:, 0], target_xyz[:, 1]
    )
