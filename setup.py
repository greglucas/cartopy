...
if conda is not None and conda in sys.prefix:
    ...
    if proj_version < PROJ_MIN_VERSION:
        print(
            ...
            "version %s."
            % (
                ".".join(str(v) for v in proj_version),
                ".".join(str(v) for v in PROJ_MIN_VERSION),
            ),
            file=sys.stderr,
        )
    ...
    try:
        proj_version = subprocess.check_output(
            ["pkg-config", "--modversion", "proj"], stderr=subprocess.STDOUT
        )
        ...
        proj_includes = [
            proj_include[2:] if proj_include.startswith("-I") else proj_include
            for proj_include in proj_includes.decode().split()
        ]
...
extensions = [
    Extension(
        ...
        include_dirs=(
            [include_dir, "./lib/cartopy", np.get_include()]
            + proj_includes
            + geos_includes
        ),
...
setup(
    ...
    package_data={
        "cartopy": list(
            file_walk_relative(
                "lib/cartopy/tests/" "mpl/baseline_images/", remove="lib/cartopy/"
            )
        )
        + list(file_walk_relative("lib/cartopy/data/raster", remove="lib/cartopy/"))
        + list(file_walk_relative("lib/cartopy/data/netcdf", remove="lib/cartopy/"))
        + list(
            file_walk_relative(
                "lib/cartopy/data/" "shapefiles/gshhs", remove="lib/cartopy/"
            )
        )
        + list(
            file_walk_relative(
                "lib/cartopy/tests/lakes_shapefile", remove="lib/cartopy/"
            )
        )
        + ["io/srtm.npz"]
    },
